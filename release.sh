		...
		if [ -f "$_cdt_source_file" ]; then
			...
			if [ -n "$_cdt_skip_copy" ]; then
				...
				if [ -n "$_cdt_only_copy" ] || ! match_pattern "$file" "*.lua:*.md:*.toc:*.txt:*.xml"; then
					...
					if [[ -z $_cdt_file_gametype && ( $file == *".lua" || $file == *".xml" || $file == *".toc" ) ]] && grep -q '@\(non-\)\?version-\(retail\|classic\|bcc\|wrath\|cata\)@' "$_cdt_source_file"; then
...
if [ -z "$skip_zipfile" ]; then
	...
	# Exclude any folder labeled "DMB-Placeholder" (as, duh, it's a placeholder)
	( cd "$releasedir" && zip -X -r "$archive" "${zip_root_dirs[@]}" -x "*DBM-Placeholder*" )
